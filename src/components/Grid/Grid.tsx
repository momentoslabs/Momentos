...
const Grid: React.FC<GridProps> = ({
  ...
  const onMouseMove = useCallback(
    (e: MouseEvent | React.MouseEvent<HTMLDivElement>) => {
      ...
      gridPos.col = Math.max(
        0,
        Math.min(GRID_COLS - moduleSize.cols, gridPos.col),
      );
      gridPos.row = Math.max(
        0,
        Math.min(GRID_ROWS - moduleSize.rows, gridPos.row),
      );
      ...
      const inBounds =
        gridPos.col >= 0 &&
        gridPos.row >= 0 &&
        gridPos.col + moduleSize.cols <= GRID_COLS &&
        gridPos.row + moduleSize.rows <= GRID_ROWS;
  ...
  const gridWidth = cellWidth * GRID_COLS;
            ...
            <div
              style={{
                ...
                width: (() => {
                  ...
                  return (
                    (sizeMap[draggingModule.data.size] || { cols: 1, rows: 1 })
                      .cols * cellWidth
                  );
                })(),
                height: (() => {
                ...
                style={{
                  ...
                  boxShadow:
                    selectedModule === module.id
                      ...
                      : "none",
                  opacity: draggingModule?.fromGrid?.id === module.id ? 0.5 : 1,
                  cursor: "default",
                  transition:
